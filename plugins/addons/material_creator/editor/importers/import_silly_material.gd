## 导入插件 —— 将 `.silly_mat_importable` 文件导入为 [SillyMaterialResource]。
...
## 返回导入后生成的资源类型。
## 注意：此处必须返回 Godot 原生类型 "Resource" 而非 GDScript 自定义类型 "SillyMaterialResource"。
...
## 核心导入函数 —— 执行实际的导入逻辑。
## 读取 `.silly_mat_importable` 文件，解析为 [SillyMaterialResource] 并保存。
##
## 参数:
##   source_file: 源文件路径
