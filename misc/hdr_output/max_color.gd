## HDR 颜色适配器（节点版） —— 将 SDR 颜色映射到 HDR 输出范围。
##
## 继承自 [Node]，监听窗口的 output_max_linear_value_changed 信号，
## 自动调整 self_modulate 使 SDR 颜色在 HDR 显示器上正确显示。
## 与 max_self_modulate.gd 功能相同，但适用于非 CanvasItem 节点的颜色属性。
extends Node
