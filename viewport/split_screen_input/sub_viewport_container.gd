...
# 上方为英文文档注释：本脚本用于为不同的 SubViewport 进行输入路由。
...
# 声明私有变量 _current_keyboard_set（当前允许的键盘按键集合），类型为 Array，初始为空数组。
# 只有该数组中包含的按键码对应的键盘事件，才会被转发到子视口。
var _current_joypad_device: int = -1
# 声明私有变量 _current_joypad_device（当前允许的手柄设备 ID），类型为 int，初始值为 -1。
...
# 英文注释：确保只有被 OptionButton 选中允许的事件，才会发送给 SubViewport。
...
func set_input_config(config_dict: Dictionary):
	# 定义公开函数 set_input_config，接收一个字典参数 config_dict。
	# 该字典通常由 SplitScreen 在切换 OptionButton 选项时生成并传入。
