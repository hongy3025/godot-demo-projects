## 碰撞对测试 —— 测试不同碰撞形状之间的碰撞检测。
extends Test


const OPTION_TYPE_BOX = "碰撞类型/盒子 (1)"
const OPTION_TYPE_SPHERE = "碰撞类型/球体 (2)"
const OPTION_TYPE_CAPSULE = "碰撞类型/胶囊 (3)"
const OPTION_TYPE_CYLINDER = "碰撞类型/圆柱 (4)"
const OPTION_TYPE_CONVEX_POLYGON = "碰撞类型/凸多边形 (5)"

const OPTION_SHAPE_BOX = "形状类型/盒子"
const OPTION_SHAPE_SPHERE = "形状类型/球体"
const OPTION_SHAPE_CAPSULE = "形状类型/胶囊"
const OPTION_SHAPE_CYLINDER = "形状类型/圆柱"
const OPTION_SHAPE_CONVEX_POLYGON = "形状类型/凸多边形"
const OPTION_SHAPE_CONCAVE_POLYGON = "形状类型/凹多边形"
...
func _do_collision_test() -> void:
	...
	Log.print_log("* 开始 %s 碰撞测试..." % shape.resource_name)
	...
	for node in $Shapes.get_children():
		...
		Log.print_log("中心内部碰撞: %s" % ("未命中" if res.is_empty() else "命中"))
...
func _find_type_index(type_name: String) -> int:
	...
	Log.print_error("无效碰撞类型: " + type_name)
	return -1
